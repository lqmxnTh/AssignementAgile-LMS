...
class AddBookTDDTest {

    private final AddBookTDD addBookGUI = new AddBookTDD();

    @Test
    public void testValidISBN() {
        assertFalse(addBookGUI.isValidISBN("123-456-789"));
        assertFalse(addBookGUI.isValidISBN("9876543210"));
    ...
    public void testAddBookToDB(){
        BookTDD bookTDD = new BookTDD( "978-0143127741","A Brief History of Humankind","Y N Harari","History","2024-05-15",5 );
        DatabaseConnectionTDD.addBook( bookTDD );
        BookTDD fetchedBook = DatabaseConnectionTDD.fetchBookByIsbn("978-0143127741");

        assertNotNull(fetchedBook);
        assertEquals("978-0143127741", fetchedBook.getIsbn());
        assertEquals("A Brief History of Humankind", fetchedBook.getTitle());
        assertEquals("Y N Harari", fetchedBook.getAuthor());
        assertEquals("History", fetchedBook.getGenre());
        assertEquals("2024-05-15", fetchedBook.getPublishedYear());
        assertEquals(5, fetchedBook.getStock());
    }
